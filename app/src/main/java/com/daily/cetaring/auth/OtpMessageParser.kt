...
object OtpMessageParser {
    ...
    private val otpBeforeKeywordPattern = Regex(
        pattern = "(?i)(?<!\\d)(\\d{6})\\D{0,24}(?:is\\s+your\\s+)?(?:otp|one\\s*time\\s*password|verification\\s*code)"
    )
    ...
    fun extractOtp(message: String): String? {
        ...
        otpBeforeKeywordPattern.find(message)?.groupValues?.getOrNull(1)?.let { return it }
